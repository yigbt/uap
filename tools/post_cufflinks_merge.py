...
def read_arguments():
    parser = argparse.ArgumentParser(
        description="Filters merged.gtf from cufflinks for novel transcripts and spits out some statistics")
    ...
    parser.add_argument(
        ...
        default=sys.stderr,
        help="Novel transcript  metrics file:  default=stderr ")

    parser.add_argument(
        '--remove-unstranded',
        action='store_true',
        default=False,
        help="Removes transcripts without strand specifity")

    parser.add_argument(
        '--remove-gencode',
        action='store_true',
        default=False,
        help="hard removal of gtf line which match 'ENS' in gene_name field")

    parser.add_argument(
        '--string',
        default='ENS',
        help="string to match in gtf field gene_name for discarding")

    parser.add_argument(
        '--remove-by-gene-name',
        action='store_true',
        default=False,
        help="remove gtf if matches string in gene_name field")

    parser.add_argument(
        '--filter-by-class',
        action='store_true',
        default=False,
        help="remove gtf if any class is found in class_code field, requieres class_list")

    parser.add_argument(
        '--class-list',
        default=None,
        help="class codes to be removed possible '=,c,j,e,i,o,p,r,u,x,s,.'")

    parser.add_argument(
        '--filter-by-class-and-gene-name',
        action='store_true',
        default=False,
        help="combines remove-by-class and remove-by-gene-name")
...
def make_transcript_object(gtf_dicts):
    ...
    for gtf_dict in gtf_dicts:
        t_obj['info']['n_exon'] += 1
        t_obj['info']['exon_lengths'].append(
            gtf_dict['end'] - gtf_dict['start'])
...
def main(args):
    ...
    for line in args.infile:
        ...
        if args.filter_by_class_and_gene_name:
            if filter_by_class_and_gene_name(
                    gtf_dict, args.string, args.class_list):
                continue
